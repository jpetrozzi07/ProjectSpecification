...
class parse_obo:
    ...
    def get_ancestors(self, term_id, visited=None): #gets ancestors/parent
        if visited is None:
    ...
    def search_menu(self): #main inteeractive menu for searching data inside the df
        ...
        print('')
        print('1. Searching for terms.')
        print('2. Viewing ontology hierarchical relations.')
        print('3. Go back to GO_Tools main menu.')
        print('')
        ...
        while True:
            selection = input('Select 1 (Searcher), 2 (Ontology viewer) or 3 to return to main menu(input an integer number): ').strip()
            ...
            if selection == '1':
                ...
                print('View:')
                print('')
                print('1. Hierarchical relations of a term (INPUT GO_ID, SHOWS ENTIRE DETAILS OF SUCH TERM)')
               ...
               break #returns to tool_menu
            
            else:
                
                print('Please insert a valid integer from 1 to 3.')
    ...
    def _display_term_details(self, term_id):
        ...
        term = self.get_term(term_id)  #these displays the details of a specific searched term (by go id)
        if term is None:
