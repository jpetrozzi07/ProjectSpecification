...
class go_id_searcher(base_searcher): #upgraded search method 1 using inheritance brom abstracted class
  ...
  #def go_id_search(self, go_id):    #search method 1
        #if self.df.empty:
            #return pd.DataFrame()
...
class GAFparser:   #class for gaf parser
    ...
    def __init__(self, filepath):  #constructor of the gaf parser class
        ...
        self.df = self._load_gaf()       #self is used by convention to reference the object currently being created
    ...
    def _load_gaf(self): #protected from external code
        ...
        try:
            print(f" Loading GAF file: {self.filepath}")
            ...
            if len(df.columns) == len(self.COLUMNS):
                ...
                print(f"Older gaf version detected ({len(df.columns)} columns)")
                df.columns = self.COLUMNS[:len(df.columns)]
            else:
                print(f"Unexpected columns: {len(df.columns)}, expected: {len(self.COLUMNS)}")
    ...
    def search_menu(self): #main inteeractive menu for searching data inside the df
        ...
        while True:
            selection = input('Select one option from 1 to 5 (input an integer number): ').strip()
            ...
            if selection == '1':
                ...
                print('Invalid option, please insert a number from 1 to 5')
    ...
    def _display_results(self, results, searcher, query):  #encapsulated so it cant be changed by external code and so i can change the displays format without breaking the code
        if results.empty:
            ...
            display_cols = ['DB', 'DB_Object_ID', 'DB_Object_Symbol', 'Qualifier','GO_ID', 'DB_Reference', 'Evidence_Code', 'With', 'Aspect', 'DB_Object_Name', 'DB_Object_Synonym',  'DB_Object_Type', 'Taxon', 'Date', 'Assigned_By',  'Annotation_Extension', 'Gene_Product_Form_ID'] #display of all the columns
